...
| `ratings/aggregate/{train,val,test}.parquet` | 1,751 / 222 / 517 | Canonical split ratings (generic attractiveness) |
...
This is **not confirmed to match the original paper's reported numbers**.
The other two legacy split generations now have the same full rigor
...
not proof of which matches the paper.
...
**The canonical score is the unweighted mean of every rating — no rater is
excluded or down-weighted.** Per-rater quality statistics ship alongside,
