...
from .protocol import Protocol, Split
...
def assert_no_test_leak(method: Method, protocol: Protocol, seed: int = 0) -> None:
    ...
    shuffled = Split(
        name=protocol.test.name,
        image_ids=protocol.test.image_ids,
        labels=rng.permutation(protocol.test.labels),
        image_paths=protocol.test.image_paths,
        distributions=protocol.test.distributions,
