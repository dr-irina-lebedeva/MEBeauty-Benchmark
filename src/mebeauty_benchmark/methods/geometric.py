...
from ..benchmark.protocol import Protocol, Split
...
def landmarks_for(protocol: Protocol) -> dict[str, np.ndarray]:
    """image_id -> (68, 2) landmark array, from the shipped parquet."""
    frame = pd.read_parquet(protocol.root / "landmarks.parquet")
    return {
        row.image_id: np.asarray(row.landmarks, dtype=float).reshape(-1, 2)
